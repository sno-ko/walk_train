...
ax_2.plot(genera_listB, simAve_log, color="red", label="cosSim_average", marker="o", markersize=1.5, linewidth=1)
...
ax_1.set_xlim(min(genera_listA), max(genera_listA)+0.5)
ax_1.set_ylim(ylim_1, np.percentile(score_log, 75)*2)

ax_2.set_xlim(min(genera_listB), max(genera_listB)+0.5)
ax_2.set_ylim(ylim_2, np.percentile(simAve_log, 75)*2)
ax_3.set_xlim(min(genera_listB), max(genera_listB)+0.5)
